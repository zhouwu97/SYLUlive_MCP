...
class WeeklyGoal(StrictInputModel):
    """需要在当前周内安排的目标及优先级。"""
...
class PlanStudentWeekInput(StrictInputModel):
    """要求内联课表和相对路径课表二选一。"""

    schedule: WeeklySchedule | None = None
    schedule_path: str | None = Field(default=None, min_length=1, max_length=500)
    goals: list[WeeklyGoal] = Field(default_factory=list, max_length=50)
    constraints: ScheduleConstraints = Field(default_factory=ScheduleConstraints)

    @model_validator(mode="after")
    def exactly_one_source(self) -> PlanStudentWeekInput:
        """拒绝缺失或重复的课表来源。"""

        has_inline = self.schedule is not None
        has_path = self.schedule_path is not None
