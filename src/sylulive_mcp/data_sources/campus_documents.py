...
class CampusDocumentRepository:
    ...
    def all_documents(self) -> list[CampusDocument]:
        """返回已通过完整性与路径检查的演示文档。"""

        return self._load_documents()

    def get_sources(self, source_ids: list[str]) -> tuple[list[CampusDocument], list[str]]:
        """按稳定来源标识重新读取资料，用于回答前后的来源状态复核。"""

        by_id = {document.source_id: document for document in self._load_documents()}
        found = [by_id[source_id] for source_id in source_ids if source_id in by_id]
